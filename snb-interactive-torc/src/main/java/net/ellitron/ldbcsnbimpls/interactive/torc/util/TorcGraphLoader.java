...
public class TorcGraphLoader {
  ...
  private static final Logger logger =
      Logger.getLogger(TorcGraphLoader.class.getName());

  private static final long TX_MAX_RETRIES = 1000;

  public static void loadVertices(TorcGraph graph, Path filePath,
      boolean printLoadingDots, int batchSize)
    ...
    for (int startIndex = 1; startIndex < lines.size();
      ...
      do {
        for (int i = startIndex; i < endIndex; i++) {
          ...
          for (int j = 0; j < colVals.length; ++j) {
            if (colNames[j].equals("id")) {
              propertiesMap.put(T.id,
                  new UInt128(Entity.fromName(entityName).getNumber(),
                      Long.decode(colVals[j])));
        ...
        if (txFailCount > TX_MAX_RETRIES) {
          throw new RuntimeException(String.format("ERROR: Transaction failed "
              + "%d times (file lines [%d,%d]), aborting...", txFailCount,
  ...
  public static void loadProperties(TorcGraph graph, Path filePath,
    ...
    for (int startIndex = 1; startIndex < lines.size(); startIndex += batchSize) {
      ...
      do {
        for (int i = startIndex; i < endIndex; i++) {
          ...
          String[] colVals = line.split("\\|");
          TorcVertex vertex =
              (TorcVertex) graph.vertices(
                  new UInt128(Entity.fromName(entityName).getNumber(),
                      Long.decode(colVals[0]))).next();
  ...
  public static void loadEdges(TorcGraph graph, Path filePath,
      boolean undirected, boolean printLoadingDots, int batchSize)
      throws IOException, java.text.ParseException {
    long count = 0;
    String[] colNames = null;
    boolean firstLine = true;
    ...
    for (int startIndex = 1; startIndex < lines.size();
      ...
      do {
        for (int i = startIndex; i < endIndex; i++) {
          ...
          TorcVertex vertex1 =
              (TorcVertex) graph.vertices(
                  new UInt128(Entity.fromName(v1EntityName).getNumber(),
                      vertex1Id)).next();
          TorcVertex vertex2 =
              (TorcVertex) graph.vertices(
                  new UInt128(Entity.fromName(v2EntityName).getNumber(),
                      vertex2Id)).next();
  ...
  public static void main(String[] args) throws IOException {
    ...
    options.addOption(null, "batchSize", true, "Number of nodes/edges to "
